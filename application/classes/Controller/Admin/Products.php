...
//   включено в getsubbrands     $this->template->scripts[] = 'public/js/genpath.js';
    ...
    public function action_index()
    {
       ...
       foreach($categories as $cat) {
         ...
         if (isset($_POST['query']))
        {
            ...
            if(!$search) $this->redirect('admin/products');
        ...
        else 
        {
            ...
            if($catid) $products->where('cat_id','=', $catid);
            $count=$products->reset(FALSE)->count_all();
            if($count) {
                $config = array(
                  'controller' => Request::current()->controller(),
                   'action' => Request::current()->action(),
                                      );
                if($catid) $config['id'] = $catid; 
            $pagination = Pagination::factory(array('total_items'=>$count,'items_per_page'=>50,'view'=> 'pagination/floating'))
                     ->route_params($config);
...
public function action_add()
{
   ...
   if(isset($_POST['add'])) {
        ...
        $data = Arr::extract($_POST, array('title','path','code','present','status','top','decor','price',
            ...
            'meta_keywords', 'meta_description','categories','foptions','attributes','unit','pieces','meters'));
        ...
        $products = ORM::factory('Product');
                    ...
                    if(isset($_POST['attributes']) && count($_POST['attributes']) > 0)
                    {
                            ...
                            $attributes['country'] = $_POST['attributes'][10];
                            //if(isset($data['attributes'])) unset($data['attributes']);
                            $data['attributes'] = $attributes;
                    }
                    $data['cat_id'] = $data['categories'][0] ? $data['categories'][0] : 0;
        $dataagrom = array();
        if(isset($data['unit'])) { $dataagrom['unit'] = $data['unit'];  }
        if(isset($data['pieces'])) { $dataagrom['pieces'] = $data['pieces'];  }
        if(isset($data['meters'])) { $dataagrom['meters'] = $data['meters'];  }
...
/////////////////////////////// Работа с изображениями ///////////////////////////
            ...
            elseif (count($image)>1 && $mainImage=="")
            {
                    foreach($image as $im => $imname)
                    {
                            ...
                            if(count($dataagrom) && array_filter($dataagrom))
                            {
                              $dataAgromat = ORM::factory('Dataagromat',$data['code']);
                              if($dataAgromat->loaded())
                              {
                                $dataAgromat->values($dataagrom);
                                $dataAgromat->save();
                              }
                              else{
                                $dataAgromat = ORM::factory('Dataagromat');
                                $dataAgromat->values($dataagrom);
                                $dataAgromat->code = $data[code];
                                $dataAgromat->save();
                              }
                            }       
...
public function action_edit()
{
  ...
  $brands = array(
   ...
   $dataagrom= array('unit'=>'','pieces'=>'','meters'=>'');
   $dtagr = ORM::factory('Dataagromat',$data['code']);
  if($dtagr->loaded()) $dataagrom = $dtagr->as_array();
   $data = $data + $dataagrom;
   ...
   if (isset($_POST['save'])) {
       $data = Arr::extract($_POST, array('title','path','code','present','status','top','decor','price',
           ...
           'meta_keywords', 'meta_description','categories','attributes','unit','pieces','meters'));
...
/////////////////////////////// Конец блока Работа с изображениями /////////////////////////// 
   ///////////////////////////// Работа с опциями фильтров  /////////////////////////////////////////////////
   if(!count($foptions)) {
       $del = DB::query(Database::DELETE, "DELETE FROM `jk_froptionvalues` WHERE `prod_id`=". $id)->execute();
   ...
   $this->template->center_block = array($content);
    ...
    public function action_delete() {
        ...
        if($prod->loaded()) {
            ...
            if($prod->image_id) {
                $delimg = DB::query(Database::DELETE, "DELETE FROM `jk_images` WHERE `product_id`=" . $id)->execute();
            }
            ...
            $dtagr = ORM::factory('Dataagromat', $prod->code);
            if ($dtagr->loaded()) $dtagr->delete();
...
public function action_setfilters()
{
     ...
     $filterstr = $this->request->param('param2');
     if(!$filterstr) die;
     $foptions = explode(';',$filterstr);
     ///////////////////////////// Работа с опциями фильтров  /////////////////////////////////////////////////
   if(!$foptions) {
           $del = DB::query(Database::DELETE, "DELETE FROM `jk_froptionvalues` WHERE `prod_id`=". $id)->execute();
   }
